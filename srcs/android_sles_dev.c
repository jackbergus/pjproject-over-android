...
static pj_status_t create_engine (struct sles_factory *sf)
{
    ...
    //
    result = slCreateEngine (&sf->e_obj, 0, NULL, 0, NULL, NULL);
    ...
    //
    result = (*sf->e_obj)->GetInterface (sf->e_obj, SL_IID_ENGINE, &sf->e_engine);
    if (result != SL_RESULT_SUCCESS) {
...
static pj_status_t open_play (struct sles_stream* stream)
{
    ...
    result = (*stream->p_obj)->GetInterface (stream->p_obj, SL_IID_PLAY, &stream->p_play);
    if (result != SL_RESULT_SUCCESS) {
...
static pj_status_t open_rec (struct sles_stream* stream)
{
    ...
    result = (*stream->r_obj)->GetInterface (stream->r_obj, SL_IID_ANDROIDCONFIGURATION, &rec_config);
    if (result != SL_RESULT_SUCCESS) {
        PJ_LOG (2,(THIS_FILE, "Error getting the recorder configuration interface"));
        return PJ_EUNKNOWN;
    }
    (*rec_config)->SetConfiguration (rec_config,
    ...
    result = (*stream->r_obj)->GetInterface (stream->r_obj, SL_IID_RECORD, &stream->r_rec);
    if (result != SL_RESULT_SUCCESS) {
